...
class DBConnection : public core::internal::CDBConnection<NativeConnection,
                                                          Config, MEMCACHED> {
public:
  typedef core::internal::CDBConnection<NativeConnection, Config, MEMCACHED>
      base_class;
  ...
  common::Error AddIfNotExist(const NKey &key, const std::string &value,
                              time_t expiration,
                              uint32_t flags) WARN_UNUSED_RESULT;
  common::Error Replace(const NKey &key, const std::string &value,
                        time_t expiration, uint32_t flags) WARN_UNUSED_RESULT;
  common::Error Append(const NKey &key, const std::string &value,
                       time_t expiration, uint32_t flags) WARN_UNUSED_RESULT;
  common::Error Prepend(const NKey &key, const std::string &value,
                        time_t expiration, uint32_t flags) WARN_UNUSED_RESULT;
  common::Error Incr(const NKey &key, uint32_t value,
                     uint64_t *result) WARN_UNUSED_RESULT;
  common::Error Decr(const NKey &key, uint32_t value,
                     uint64_t *result) WARN_UNUSED_RESULT;
...
private:
  common::Error CheckResultCommand(const std::string &cmd,
                                   int err) WARN_UNUSED_RESULT;

  common::Error DelInner(const key_t &key,
                         time_t expiration) WARN_UNUSED_RESULT;
  common::Error GetInner(const key_t &key,
                         std::string *ret_val) WARN_UNUSED_RESULT;
  common::Error SetInner(const key_t &key, const value_t &value,
                         time_t expiration, uint32_t flags) WARN_UNUSED_RESULT;
