...
class DBConnection
    : public core::internal::CDBConnection<NativeConnection, Config,
                                           connection_type> {
public:
  typedef std::shared_ptr<CommandTranslator> redis_translator_t;
  typedef core::internal::CDBConnection<NativeConnection, Config,
                                        connection_type>
      base_class;
  ...
  explicit DBConnection(CDBConnectionClient *client)
      : base_class(client, new CommandTranslator(base_class::GetCommands())),
        is_auth_(false), cur_db_(invalid_db_num) {}
  ...
  common::Error CommonExec(const commands_args_t &argv,
  ...
  common::Error Lpush(const NKey &key, NValue arr,
                      long long *list_len) WARN_UNUSED_RESULT;
  common::Error Lrange(const NKey &key, int start, int stop,
                       NDbKValue *loaded_key) WARN_UNUSED_RESULT;

  common::Error Mget(const std::vector<NKey> &keys,
                     std::vector<NDbKValue> *loaded_keys) WARN_UNUSED_RESULT;
  common::Error Mset(const std::vector<NDbKValue> &keys,
                     std::vector<NDbKValue> *added_key) WARN_UNUSED_RESULT;
  common::Error MsetNX(const std::vector<NDbKValue> &keys,
                       long long *result) WARN_UNUSED_RESULT;
  ...
  common::Error PExpire(const NKey &key,
                        ttl_t ttl)
      WARN_UNUSED_RESULT; // PEXPIRE works differently than in redis protocol
  ...
  common::Error ExecuteAsPipeline(
      const std::vector<FastoObjectCommandIPtr> &cmds,
      void (*log_command_cb)(FastoObjectCommandIPtr)) WARN_UNUSED_RESULT;

protected:
  common::Error CliFormatReplyRaw(FastoObject *out,
                                  redisReply *r) WARN_UNUSED_RESULT;
...
private:
  ...
  virtual common::Error GetImpl(const NKey &key,
                                NDbKValue *loaded_key)
      override; // GET works differently than in redis protocol
  virtual common::Error RenameImpl(const NKey &key,
                                   const key_t &new_key) override;
  virtual common::Error SetTTLImpl(
      const NKey &key,
