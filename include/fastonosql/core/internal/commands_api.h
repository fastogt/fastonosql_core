...
template <class CDBConnection> struct ApiTraits {
  typedef CDBConnection cdb_connection_t;
  static common::Error Help(CommandHandler *handler, commands_args_t argv,
                            FastoObject *out);

  static common::Error Scan(CommandHandler *handler, commands_args_t argv,
                            FastoObject *out);
  static common::Error Keys(CommandHandler *handler, commands_args_t argv,
                            FastoObject *out);
  static common::Error DBkcount(CommandHandler *handler, commands_args_t argv,
                                FastoObject *out);
  ...
  static common::Error RemoveDatabase(CommandHandler *handler,
                                      commands_args_t argv, FastoObject *out);
  static common::Error FlushDB(CommandHandler *handler, commands_args_t argv,
                               FastoObject *out);
  static common::Error Select(CommandHandler *handler, commands_args_t argv,
                              FastoObject *out);
  static common::Error Set(CommandHandler *handler, commands_args_t argv,
                           FastoObject *out);
  static common::Error Get(CommandHandler *handler, commands_args_t argv,
                           FastoObject *out);
  static common::Error Rename(CommandHandler *handler, commands_args_t argv,
                              FastoObject *out);
  static common::Error Delete(CommandHandler *handler, commands_args_t argv,
                              FastoObject *out);
  static common::Error SetTTL(CommandHandler *handler, commands_args_t argv,
                              FastoObject *out);
  static common::Error GetTTL(CommandHandler *handler, commands_args_t argv,
                              FastoObject *out);
  static common::Error Quit(CommandHandler *handler, commands_args_t argv,
                            FastoObject *out);
  static common::Error ConfigGet(internal::CommandHandler *handler,
                                 commands_args_t argv, FastoObject *out);
  static common::Error JsonDump(CommandHandler *handler, commands_args_t argv,
                                FastoObject *out);
};
...
common::Error ApiTraits<CDBConnection>::Scan(internal::CommandHandler *handler,
  ...
  common::Error err =
      cdb->Scan(cursor_in, pattern, count_keys, &keys_out, &cursor_out);
  if (err) {
    return err;
  }
  ...
  std::string rep_out =
      common::ConvertToString(cursor_out); // string representing
...
template <class CDBConnection>
common::Error
ApiTraits<CDBConnection>::DBkcount(internal::CommandHandler *handler,
                                   commands_args_t argv, FastoObject *out) {
  UNUSED(argv);
...
template <class CDBConnection>
common::Error
ApiTraits<CDBConnection>::FlushDB(internal::CommandHandler *handler,
                                  commands_args_t argv, FastoObject *out) {
  UNUSED(argv);
...
common::Error ApiTraits<CDBConnection>::Set(internal::CommandHandler *handler,
  ...
  NValue string_val(
      common::Value::CreateStringValue(common::ConvertToString(argv[1])));
...
template <class CDBConnection>
common::Error
ApiTraits<CDBConnection>::Delete(internal::CommandHandler *handler,
                                 commands_args_t argv, FastoObject *out) {
  NKeys keysdel;
  ...
  common::FundamentalValue *val =
      common::Value::CreateUIntegerValue(keys_deleted.size());
  FastoObject *child = new FastoObject(out, val, cdb->GetDelimiter());
  out->AddChildren(child);
  return common::Error();
}
...
template <class CDBConnection>
common::Error
ApiTraits<CDBConnection>::Rename(internal::CommandHandler *handler,
                                 commands_args_t argv, FastoObject *out) {
  key_t raw_key(argv[0]);
  NKey key(raw_key);
...
template <class CDBConnection>
common::Error
ApiTraits<CDBConnection>::SetTTL(internal::CommandHandler *handler,
                                 commands_args_t argv, FastoObject *out) {
  CDBConnection *cdb = static_cast<CDBConnection *>(handler);
  key_t raw_key(argv[0]);
  NKey key(raw_key);
...
template <class CDBConnection>
common::Error
ApiTraits<CDBConnection>::GetTTL(internal::CommandHandler *handler,
                                 commands_args_t argv, FastoObject *out) {
  CDBConnection *cdb = static_cast<CDBConnection *>(handler);
  key_t raw_key(argv[0]);
  NKey key(raw_key);
...
template <class CDBConnection>
common::Error
ApiTraits<CDBConnection>::ConfigGet(internal::CommandHandler *handler,
                                    commands_args_t argv, FastoObject *out) {
...
template <class CDBConnection>
common::Error ApiTraits<CDBConnection>::JsonDump(CommandHandler *handler,
                                                 commands_args_t argv,
                                                 FastoObject *out) {
  cursor_t cursor_in;
  const size_t argc = argv.size();
  ...
  CDBConnection *cdb = static_cast<CDBConnection *>(handler);
  common::Error err =
      cdb->JsonDump(cursor_in, pattern, count_keys, path, &cursor_out);
  if (err) {
    return err;
  }
