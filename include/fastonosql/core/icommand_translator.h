...
public:
  ...
  common::Error
  GetDatabasesCommand(command_buffer_t *cmdstring) const WARN_UNUSED_RESULT;
  common::Error
  RemoveDBCommand(const std::string &name,
                  command_buffer_t *cmdstring) const WARN_UNUSED_RESULT;
  common::Error
  CreateDBCommand(const std::string &name,
                  command_buffer_t *cmdstring) const WARN_UNUSED_RESULT;

  common::Error
  SelectDBCommand(const std::string &name,
                  command_buffer_t *cmdstring) const WARN_UNUSED_RESULT;
  common::Error
  FlushDBCommand(command_buffer_t *cmdstring) const WARN_UNUSED_RESULT;
  common::Error
  CreateKeyCommand(const NDbKValue &key,
                   command_buffer_t *cmdstring) const WARN_UNUSED_RESULT;
  common::Error
  LoadKeyCommand(const NKey &key, common::Value::Type type,
                 command_buffer_t *cmdstring) const WARN_UNUSED_RESULT;
  common::Error
  DeleteKeyCommand(const NKey &key,
                   command_buffer_t *cmdstring) const WARN_UNUSED_RESULT;
  common::Error
  RenameKeyCommand(const NKey &key, const key_t &new_name,
                   command_buffer_t *cmdstring) const WARN_UNUSED_RESULT;
  common::Error
  ChangeKeyTTLCommand(const NKey &key, ttl_t ttl,
                      command_buffer_t *cmdstring) const WARN_UNUSED_RESULT;
  common::Error
  LoadKeyTTLCommand(const NKey &key,
                    command_buffer_t *cmdstring) const WARN_UNUSED_RESULT;

  bool IsLoadKeyCommand(const command_buffer_t &cmd,
                        readable_string_t *key) const WARN_UNUSED_RESULT;

  common::Error
  PublishCommand(const NDbPSChannel &channel, const std::string &message,
                 command_buffer_t *cmdstring) const WARN_UNUSED_RESULT;

  common::Error
  SubscribeCommand(const NDbPSChannel &channel,
                   command_buffer_t *cmdstring) const WARN_UNUSED_RESULT;

  std::vector<CommandInfo> GetCommands() const;
  common::Error
  FindCommand(const std::string &command_first_name,
              const CommandHolder **info) const WARN_UNUSED_RESULT;
  common::Error FindCommand(commands_args_t argv, const CommandHolder **info,
                            size_t *off) const WARN_UNUSED_RESULT;

  common::Error TestCommandArgs(const CommandHolder *cmd,
                                commands_args_t argv) const WARN_UNUSED_RESULT;
  common::Error
  TestCommandLine(const command_buffer_t &cmd) const WARN_UNUSED_RESULT;
  common::Error TestCommandLineArgs(commands_args_t argv,
                                    const CommandHolder **info,
                                    size_t *off) const WARN_UNUSED_RESULT;

  static common::Error
  InvalidInputArguments(const std::string &cmd) WARN_UNUSED_RESULT;
  static common::Error NotSupported(const std::string &cmd) WARN_UNUSED_RESULT;
  static common::Error
  UnknownCommand(const std::string &cmd) WARN_UNUSED_RESULT;
...
private:
  ...
  LoadKeyCommandImpl(const NKey &key, common::Value::Type type,
                     command_buffer_t *cmdstring) const = 0;
  virtual common::Error
  DeleteKeyCommandImpl(const NKey &key, command_buffer_t *cmdstring) const = 0;
  virtual common::Error
  ...
  ChangeKeyTTLCommandImpl(const NKey &key, ttl_t ttl,
                          command_buffer_t *cmdstring) const = 0;
  virtual common::Error
  LoadKeyTTLCommandImpl(const NKey &key, command_buffer_t *cmdstring) const = 0;
  virtual common::Error
