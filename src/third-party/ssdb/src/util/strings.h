...
inline static void str_escape(const char *s, int size, std::string *ret) {
...
inline static std::string str_escape(const char *s, int size) {
  std::string ret;
...
inline static std::string str_escape(const std::string &s) {
...
inline static void str_unescape(const char *s, int size, std::string *ret) {
  for (int i = 0; i < size; i++) {
    char c = s[i];
    if (c != '\\') {
      ret->push_back(c);
...
inline static std::string str_unescape(const char *s, int size) {
  std::string ret;
  ret.reserve(size);
...
inline static std::string str_unescape(const std::string &s) {
...
static inline std::string str(const char *s) { return std::string(s); }

static inline std::string str(int v) {
  char buf[21] = {0};
...
static inline std::string str(uint64_t v) {
  char buf[21] = {0};
  snprintf(buf, sizeof(buf), "%" PRIu64 "", v);
  return std::string(buf);
}

static inline std::string str(double v) {
  char buf[21] = {0};
  if (v - floor(v) == 0) {
...
static inline std::string str(float v) { return str((double)v); }
...
static inline uint64_t str_to_uint64(const char *p, int size) {
...
static inline std::string substr(const std::string &str, int start, int size) {
  if (start < 0) {
...
static inline std::string str_slice(const std::string &str, int start,
                                    int end) {
  if (start < 0) {
    start = (int)str.size() + start;
  }
...
static inline int bitcount(const char *p, int size) {
  int n = 0;
  for (int i = 0; i < size; i++) {
...
#else
static inline uint16_t big_endian(uint16_t v) { return (v >> 8) | (v << 8); }
