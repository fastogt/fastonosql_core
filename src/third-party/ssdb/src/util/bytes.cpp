...
std::string Buffer::stats() const {
  char str[1024 * 32];
  str[0] = '\n';
  sprintf(str, "total: %d, data: %d, size: %d, slot: %d", total_,
          (int)(data_ - buf), size_, (int)(slot() - buf));
...
int Buffer::append(const char *p) { return this->append(p, strlen(p)); }

int Buffer::append(const Bytes &s) { return this->append(s.data(), s.size()); }
