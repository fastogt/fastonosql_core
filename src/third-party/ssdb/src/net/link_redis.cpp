...
enum REPLY { REPLY_BULK = 0, REPLY_MULTI_BULK, REPLY_INT, REPLY_STATUS };
...
static RedisCommand_raw cmds_raw[] = {
    ...
    {STRATEGY_REMRANGEBYSCORE, "zremrangebyscore", "zremrangebyscore",
     REPLY_INT},
...
int RedisLink::convert_req() {
  ...
  if (this->req_desc->strategy == STRATEGY_ZRANGEBYSCORE ||
      this->req_desc->strategy == STRATEGY_ZREVRANGEBYSCORE) {
    recv_string.push_back(req_desc->ssdb_cmd);
...
int RedisLink::send_resp(Buffer *output, const std::vector<std::string> &resp) {
  ...
  if (req_desc->strategy == STRATEGY_MGET ||
      req_desc->strategy == STRATEGY_HMGET) {
    if (resp.size() % 2 != 1) {
      output->append("*0");
  ...
  if (req_desc->reply_type == REPLY_MULTI_BULK) {
    ...
    if (req_desc->strategy == STRATEGY_ZRANGE ||
        req_desc->strategy == STRATEGY_ZREVRANGE) {
      if (recv_string.size() < 5 || recv_string[4] != "withscores") {
        withscores = false;
      }
    }
    if (req_desc->strategy == STRATEGY_ZRANGEBYSCORE ||
        req_desc->strategy == STRATEGY_ZREVRANGEBYSCORE) {
