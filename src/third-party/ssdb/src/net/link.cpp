...
Link *Link::accept() {
  ...
  while ((client_sock = ::accept(sock, (struct sockaddr *)&addr, &addrlen)) ==
         -1) {
    if (errno != F_EINTR) {
...
#ifdef FASTO
#ifdef OS_WIN
  int ret = ::setsockopt(client_sock, SOL_SOCKET, SO_LINGER, (const char *)&opt,
                         sizeof(opt));
#else
  int ret = ::setsockopt(client_sock, SOL_SOCKET, SO_LINGER, (void *)&opt,
                         sizeof(opt));
#endif
#else
  int ret = ::setsockopt(client_sock, SOL_SOCKET, SO_LINGER, (void *)&opt,
...
#endif
  if (ret != 0) {
    // log_error("socket %d set linger failed: %s", client_sock,
    // strerror(errno));
  }
  ...
  while ((want = input->space()) > 0) {
    ...
    if (len == -1) {
      if (errno == F_EINTR) {
        ...
        // log_debug("fd: %d, read: -1, want: %d, error: %s", sock, want,
        // strerror(errno));
        return -1;
      }
    } else {
...
const std::vector<Bytes> *Link::recv() {
  ...
  while (size > 0) {
    ...
    // log_debug("size: %d, head_len: %d, body_len: %d", size, head_len,
    // body_len);
    ...
    if (parsed > MAX_PACKET_SIZE) {
      // log_warn("fd: %d, exceed max packet size, parsed: %d", this->sock,
      // parsed);
      return NULL;
    }
  }
  ...
  if (input->space() == 0) {
    ...
    if (input->space() == 0) {
      ...
      // log_debug("fd: %d, resize input buffer, %s", this->sock,
      // input->stats().c_str());
    }
  }
