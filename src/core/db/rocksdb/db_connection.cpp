...
const ConstantCommandsArray g_commands = {
    ...
    CommandHolder("MGET", "<key> [key ...]", "Get the value of a key.",
    ...
    CommandHolder("MERGE", "<key> <value>",
                  ...
                  CommandInfo::Native, &CommandsApi::Merge),
...
template <> const char *ConnectionTraits<ROCKSDB>::GetBasedOn() {
  return "rocksdb";
}

template <> const char *ConnectionTraits<ROCKSDB>::GetVersionApi() {
  return STRINGIZE(ROCKSDB_MAJOR) "." STRINGIZE(ROCKSDB_MINOR) "." STRINGIZE(
      ROCKSDB_PATCH);
}
...
template <>
common::Error
ConnectionAllocatorTraits<rocksdb::NativeConnection, rocksdb::Config>::Connect(
    const rocksdb::Config &config, rocksdb::NativeConnection **hout) {
...
template <>
common::Error
ConnectionAllocatorTraits<rocksdb::NativeConnection, rocksdb::Config>::
    Disconnect(rocksdb::NativeConnection **handle) {
  destroy(handle);
  return common::Error();
}

template <>
bool ConnectionAllocatorTraits<rocksdb::NativeConnection, rocksdb::Config>::
    IsConnected(rocksdb::NativeConnection *handle) {
  if (!handle) {
    return false;
  }
...
public:
  ...
  ::rocksdb::ColumnFamilyHandle *GetCurrentColumn() const {
    return handles_[current_db_index_];
  }
  ...
  ::rocksdb::Status CreateDB(const std::string &name) {
    ...
    ::rocksdb::Status st =
        db_->CreateColumnFamily(::rocksdb::ColumnFamilyOptions(), name, &fam);
    if (st.ok()) {
...
common::Error CreateConnection(const Config &config,
  ...
  if (!st.ok()) {
    if (!rs.create_if_missing) {
      std::string buff =
          common::MemSPrintf("Fail open database: %s!", st.ToString());
  ...
  for (size_t i = 0; i < column_families_str.size(); ++i) {
    ::rocksdb::ColumnFamilyDescriptor descr(column_families_str[i],
...
common::Error DBConnection::GetInner(const key_t &key, std::string *ret_val) {
  ...
  const ::rocksdb::Slice key_slice(
      reinterpret_cast<const char *>(key_str.data()), key_str.size());
  return CheckResultCommand(DB_GET_KEY_COMMAND,
                            connection_.handle_->Get(ro, key_slice, ret_val));
}
...
common::Error DBConnection::Merge(const std::string &key,
  ...
  return CheckResultCommand("MERGE",
                            connection_.handle_->Merge(wo, key, value));
}
...
common::Error DBConnection::SetInner(const key_t &key, const value_t &value) {
  ...
  return CheckResultCommand(DB_SET_KEY_COMMAND,
                            connection_.handle_->Put(wo, key_slice, value_str));
}
...
common::Error DBConnection::DelInner(const key_t &key) {
  ...
  return CheckResultCommand(DB_DELETE_KEY_COMMAND,
                            connection_.handle_->Delete(wo, key_slice));
}
...
common::Error DBConnection::ScanImpl(cursor_t cursor_in,
                                     ...
                                     cursor_t *cursor_out) {
  ::rocksdb::ReadOptions ro;
  ::rocksdb::Iterator *it = connection_.handle_->NewIterator(
      ro); // keys(key_start, key_end, limit, ret);
...
common::Error DBConnection::FlushDBImpl() {
  ...
  for (it->SeekToFirst(); it->Valid(); it->Next()) {
    ...
    common::Error err = CheckResultCommand(
        DB_FLUSHDB_COMMAND, connection_.handle_->Delete(wo, key));
    if (err) {
      delete it;
      return err;
    }
...
common::Error DBConnection::CreateDBImpl(const std::string &name,
                                         IDataBaseInfo **info) {
  common::Error err = CheckResultCommand(DB_CREATEDB_COMMAND,
                                         connection_.handle_->CreateDB(name));
  if (err) {
    return err;
  }
...
common::Error DBConnection::RemoveDBImpl(const std::string &name,
                                         IDataBaseInfo **info) {
  common::Error err = CheckResultCommand(DB_REMOVEDB_COMMAND,
                                         connection_.handle_->RemoveDB(name));
  if (err) {
    return err;
  }
...
common::Error DBConnection::SelectImpl(const std::string &name,
                                       IDataBaseInfo **info) {
  common::Error err = CheckResultCommand(DB_SELECTDB_COMMAND,
                                         connection_.handle_->Select(name));
  if (err) {
    return err;
  }
