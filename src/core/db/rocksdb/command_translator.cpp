...
common::Error CommandTranslator::CreateKeyCommandImpl(const NDbKValue& key, command_buffer_t* cmdstring) const {
  ...
  wr << ROCKSDB_SET_KEY_COMMAND SPACE_STR << key_str.GetForCommandLine() << SPACE_STR << value_str.GetForCommandLine();
  *cmdstring = wr.str();
  return common::Error();
}
...
common::Error CommandTranslator::LoadKeyCommandImpl(const NKey& key,
  ...
  command_buffer_writer_t wr;
  wr << ROCKSDB_GET_KEY_COMMAND SPACE_STR << key_str.GetForCommandLine();
  *cmdstring = wr.str();
  return common::Error();
}
...
common::Error CommandTranslator::DeleteKeyCommandImpl(const NKey& key, command_buffer_t* cmdstring) const {
  key_t key_str = key.GetKey();
  command_buffer_writer_t wr;
  wr << ROCKSDB_DELETE_KEY_COMMAND SPACE_STR << key_str.GetForCommandLine();
  *cmdstring = wr.str();
  return common::Error();
}
...
common::Error CommandTranslator::RenameKeyCommandImpl(const NKey& key,
  ...
  key_t key_str = key.GetKey();
  command_buffer_writer_t wr;
  wr << ROCKSDB_RENAME_KEY_COMMAND SPACE_STR << key_str.GetForCommandLine() << SPACE_STR
