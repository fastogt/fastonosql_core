...
bool ConvertFromString(const readable_string_t& value, common::Value::Type* out) {
  ...
  } else if (value == GEN_CMD_STRING("TSDB-TYPE")) {
    lout = core::TimeSeriesValue::TYPE_TSDB;
  }
...
bool ConvertFromType(common::Value::Type type, readable_string_t* out) {
  ...
  } else if (type == core::TimeSeriesValue::TYPE_TSDB) {
    lout = GEN_CMD_STRING("TSDB-TYPE");
  }
...
common::Error CreateConnection(const Config& config, const SSHInfo& sinfo, NativeConnection** context) {
  ...
  if (is_local) {
    const char* hostsocket = config.hostsocket.c_str();
...
template <typename Config, ConnectionType ContType>
common::Error DBConnection<Config, ContType>::ZpopMax(const NKey& key, size_t count, common::ArrayValue** poped) {
  if (!poped) {
    DNOTREACHED();
    return common::make_error_inval();
  }
  ...
  redisReply* reply = nullptr;
  err = redis_compatible::ExecRedisCommand(base_class::connection_.handle_, zpopmax_cmd, &reply);
  if (err) {
    return err;
  }
  ...
  if (reply->type == REDIS_REPLY_NIL) {
    ...
    return base_class::GenerateError("ZPOPMAX", "key not found.");
  }
  ...
  if (base_class::client_ && arr->IsEmpty()) {
    base_class::client_->OnRemovedKeys({key});
  }
...
template <typename Config, ConnectionType ContType>
common::Error DBConnection<Config, ContType>::ZpopMin(const NKey& key, size_t count, common::ArrayValue** poped) {
  if (!poped) {
    DNOTREACHED();
    return common::make_error_inval();
  }
  ...
  redisReply* reply = nullptr;
  err = redis_compatible::ExecRedisCommand(base_class::connection_.handle_, zpopmin_cmd, &reply);
  if (err) {
    return err;
  }
  ...
  if (reply->type == REDIS_REPLY_NIL) {
    // key_t key_str = key.GetKey();
    return base_class::GenerateError("ZPOPMIN", "key not found.");
  }
...
common::Error DBConnection<Config, ContType>::HgetallImpl(const NKey& key, NDbKValue* loaded_key) {
  ...
  for (size_t i = 0; i < arr->GetSize(); i += 2) {
    ...
    if (arr->Get(i, &lkey) && arr->Get(i + 1, &lvalue)) {
      ...
      if (lkey->GetAsString(&key)) {
        hash->Insert(key, lvalue->DeepCopy());
      }
    }
  }
...
template <typename Config, ConnectionType ContType>
common::Error DBConnection<Config, ContType>::Unlink(const NKeys& keys, NKeys* deleted_keys) {
...
#endif
#if defined(BUILD_WITH_KEYDB)
template class DBConnection<keydb::RConfig, KEYDB>;
#endif
}  // namespace redis_compatible
}  // namespace core
}  // namespace fastonosql
