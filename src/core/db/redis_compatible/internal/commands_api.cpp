...
template <typename DBConnection>
common::Error CommandsApi<DBConnection>::ZpopMin(internal::CommandHandler* handler,
                                                 commands_args_t argv,
                                                 FastoObject* out) {
...
template <typename DBConnection>
common::Error CommandsApi<DBConnection>::ZpopMax(internal::CommandHandler* handler,
                                                 commands_args_t argv,
                                                 FastoObject* out) {
...
common::Error CommandsApi<DBConnection>::Hmset(internal::CommandHandler* handler,
  ...
  for (size_t i = 1; i < argv.size(); i += 2) {
    ...
    hmset->Insert(key, common::Value::CreateStringValue(val));
  }
...
namespace redis_compatible {
#if defined(BUILD_WITH_REDIS)
template struct CommandsApi<redis::DBConnection>;
#endif
#if defined(BUILD_WITH_PIKA)
template struct CommandsApi<pika::DBConnection>;
#endif
#if defined(BUILD_WITH_DYNOMITE)
template struct CommandsApi<dynomite::DBConnection>;
#endif
#if defined(BUILD_WITH_KEYDB)
template struct CommandsApi<keydb::DBConnection>;
#endif
}  // namespace redis_compatible
}  // namespace core
}  // namespace fastonosql
