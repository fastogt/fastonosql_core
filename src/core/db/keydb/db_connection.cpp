...
#include "core/db/keydb/internal/cluster_infos.h"
#include "core/db/keydb/internal/sentinel_info.h"
#endif
...
const ConstantCommandsArray kCommands = {
    ...
    CommandHolder(GEN_CMD_STRING(DB_INFO_COMMAND),
                  ...
                  PROJECT_VERSION_GENERATE(0, 9, 2),
...
template <>
const char* ConnectionTraits<KEYDB>::GetBasedOn() {
  return "hiredis";
}

template <>
const char* ConnectionTraits<KEYDB>::GetVersionApi() {
  return redis_compatible::GetHiredisVersion();
}

template <>
const ConstantCommandsArray& ConnectionCommandsTraits<KEYDB>::GetCommands() {
  return keydb::kCommands;
}

namespace internal {
template <>
common::Error Connection<keydb::NativeConnection, keydb::RConfig>::Connect(const keydb::RConfig& config,
                                                                           keydb::NativeConnection** hout) {
  keydb::NativeConnection* context = nullptr;
  common::Error err = keydb::CreateConnection(config, &context);
  if (err) {
    return err;
  }
...
template <>
common::Error Connection<keydb::NativeConnection, keydb::RConfig>::Disconnect(keydb::NativeConnection** handle) {
  keydb::NativeConnection* lhandle = *handle;
  if (lhandle) {
    redisFree(lhandle);
  }
  lhandle = nullptr;
...
template <>
bool Connection<keydb::NativeConnection, keydb::RConfig>::IsConnected(keydb::NativeConnection* handle) {
  if (!handle) {
    return false;
  }
