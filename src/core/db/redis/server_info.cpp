...
const std::vector<Field> g_redis_client_fields = {
    ...
    Field(REDIS_CLIENTS_CLIENT_LONGEST_OUTPUT_LIST_LABEL,
          common::Value::TYPE_UINTEGER),
    Field(REDIS_CLIENTS_CLIENT_BIGGEST_INPUT_BUF_LABEL,
          common::Value::TYPE_UINTEGER),
...
const std::vector<Field> g_redis_memory_fields = {
    ...
    Field(REDIS_MEMORY_USED_MEMORY_PEAK_HUMAN_LABEL,
          common::Value::TYPE_STRING),
    Field(REDIS_MEMORY_USED_MEMORY_LUA_LABEL, common::Value::TYPE_UINTEGER),
    Field(REDIS_MEMORY_MEM_FRAGMENTATION_RATIO_LABEL,
...
const std::vector<Field> g_redis_persistence_fields = {
    ...
    Field(REDIS_PERSISTENCE_RDB_CHANGES_SINCE_LAST_SAVE_LABEL,
          common::Value::TYPE_UINTEGER),
    Field(REDIS_PERSISTENCE_RDB_DGSAVE_IN_PROGRESS_LABEL,
          common::Value::TYPE_UINTEGER),
    Field(REDIS_PERSISTENCE_RDB_LAST_SAVE_TIME_LABEL,
          common::Value::TYPE_UINTEGER),
    Field(REDIS_PERSISTENCE_RDB_LAST_DGSAVE_STATUS_LABEL,
          common::Value::TYPE_STRING),
    Field(REDIS_PERSISTENCE_RDB_LAST_DGSAVE_TIME_SEC_LABEL,
          common::Value::TYPE_INTEGER),
    Field(REDIS_PERSISTENCE_RDB_CURRENT_DGSAVE_TIME_SEC_LABEL,
          common::Value::TYPE_INTEGER),
    Field(REDIS_PERSISTENCE_AOF_ENABLED_LABEL, common::Value::TYPE_UINTEGER),
    Field(REDIS_PERSISTENCE_AOF_REWRITE_IN_PROGRESS_LABEL,
          common::Value::TYPE_UINTEGER),
    Field(REDIS_PERSISTENCE_AOF_REWRITE_SHEDULED_LABEL,
          common::Value::TYPE_UINTEGER),
    Field(REDIS_PERSISTENCE_AOF_LAST_REWRITE_TIME_SEC_LABEL,
          common::Value::TYPE_INTEGER),
    Field(REDIS_PERSISTENCE_AOF_CURRENT_REWRITE_TIME_SEC_LABEL,
          common::Value::TYPE_INTEGER),
    Field(REDIS_PERSISTENCE_AOF_LAST_DGREWRITE_STATUS_LABEL,
          common::Value::TYPE_STRING),
    Field(REDIS_PERSISTENCE_AOF_LAST_WRITE_STATUS_LABEL,
          common::Value::TYPE_STRING)};

const std::vector<Field> g_redis_stats_fields = {
    Field(REDIS_STATS_TOTAL_CONNECTIONS_RECEIVED_LABEL,
          common::Value::TYPE_UINTEGER),
    Field(REDIS_STATS_TOTAL_COMMANDS_PROCESSED_LABEL,
          common::Value::TYPE_UINTEGER),
    Field(REDIS_STATS_INSTANTANEOUS_OPS_PER_SEC_LABEL,
          common::Value::TYPE_UINTEGER),
...
const std::vector<Field> g_redis_replication_fields = {
    ...
    Field(REDIS_REPLICATION_CONNECTED_SLAVES_LABEL,
          common::Value::TYPE_UINTEGER),
    Field(REDIS_REPLICATION_MASTER_REPL_OFFSET_LABEL,
          common::Value::TYPE_UINTEGER),
    ...
    Field(REDIS_REPLICATION_BACKLOG_FIRST_BYTE_OFFSET_LABEL,
          common::Value::TYPE_UINTEGER),
    Field(REDIS_REPLICATION_BACKLOG_HISTEN_LABEL,
          common::Value::TYPE_UINTEGER)};
...
const std::vector<Field> g_redis_cpu_fields = {
    ...
    Field(REDIS_CPU_USED_CPU_USER_CHILDREN_LABEL,
          common::Value::TYPE_UINTEGER)};
...
std::vector<common::Value::Type> DBTraits<REDIS>::GetSupportedValueTypes() {
  return {common::Value::TYPE_BOOLEAN,  common::Value::TYPE_INTEGER,
          ...
          common::Value::TYPE_ARRAY,    common::Value::TYPE_SET,
          common::Value::TYPE_ZSET,     common::Value::TYPE_HASH,
          JsonValue::TYPE_JSON,         StreamValue::TYPE_STREAM};
}

template <> std::vector<info_field_t> DBTraits<REDIS>::GetInfoFields() {
...
ServerInfo::Server::Server::Server()
    : redis_version_(), redis_git_sha1_(), redis_git_dirty_(), redis_mode_(),
      os_(), arch_bits_(0), multiplexing_api_(), gcc_version_(), process_id_(0),
      run_id_(), tcp_port_(0), uptime_in_seconds_(0), uptime_in_days_(0),
      hz_(0), lru_clock_(0) {}
...
ServerInfo::Server::Server(const std::string &server_text)
  ...
  size_t pos = 0;
  size_t start = 0;
  while ((pos = server_text.find(REDIS_INFO_MARKER, start)) !=
         std::string::npos) {
...
ServerInfo::Clients::Clients(const std::string &client_text)
  ...
  size_t pos = 0;
  size_t start = 0;
  while ((pos = client_text.find(REDIS_INFO_MARKER, start)) !=
         std::string::npos) {
...
ServerInfo::Memory::Memory()
    : used_memory_(0), used_memory_human_(), used_memory_rss_(0),
      used_memory_peak_(0), used_memory_peak_human_(), used_memory_lua_(0),
      mem_fragmentation_ratio_(0), mem_allocator_() {}
...
ServerInfo::Memory::Memory(const std::string &memory_text)
  ...
  size_t pos = 0;
  size_t start = 0;
  while ((pos = memory_text.find(REDIS_INFO_MARKER, start)) !=
         std::string::npos) {
...
ServerInfo::Persistence::Persistence(const std::string &persistence_text)
  ...
  size_t pos = 0;
  size_t start = 0;
  while ((pos = persistence_text.find(REDIS_INFO_MARKER, start)) !=
         std::string::npos) {
...
common::Value *
ServerInfo::Persistence::GetValueByIndex(unsigned char index) const {
  switch (index) {
  case 0:
...
ServerInfo::Stats::Stats()
    : total_connections_received_(0), total_commands_processed_(0),
      instantaneous_ops_per_sec_(0), rejected_connections_(0), sync_full_(0),
      sync_partial_ok_(0), sync_partial_err_(0), expired_keys_(0),
      evicted_keys_(0), keyspace_hits_(0), keyspace_misses_(0),
      pubsub_channels_(0), pubsub_patterns_(0), latest_fork_usec_(0) {}
...
ServerInfo::Stats::Stats(const std::string &stats_text)
  ...
  size_t pos = 0;
  size_t start = 0;
  while ((pos = stats_text.find(REDIS_INFO_MARKER, start)) !=
         std::string::npos) {
...
ServerInfo::Replication::Replication()
    : role_(), connected_slaves_(0), master_repl_offset_(0), backlog_active_(0),
      backlog_size_(0), backlog_first_byte_offset_(0), backlog_histen_(0) {}
...
ServerInfo::Replication::Replication(const std::string &replication_text)
  ...
  while ((pos = replication_text.find(REDIS_INFO_MARKER, start)) !=
         std::string::npos) {
...
common::Value *
ServerInfo::Replication::GetValueByIndex(unsigned char index) const {
  switch (index) {
  case 0:
...
ServerInfo::Cpu::Cpu()
    : used_cpu_sys_(0), used_cpu_user_(0), used_cpu_sys_children_(0),
...
common::Value *
ServerInfo::Keyspace::GetValueByIndex(unsigned char index) const {
  UNUSED(index);
...
ServerInfo::ServerInfo(const Server &serv, const Clients &clients,
                       const Memory &memory, const Persistence &pers,
                       const Stats &stats, const Replication &repl,
                       const Cpu &cpu, const Keyspace &key)
    : IServerInfo(REDIS), server_(serv), clients_(clients), memory_(memory),
      persistence_(pers), stats_(stats), replication_(repl), cpu_(cpu),
      keySp_(key) {}
...
std::ostream &operator<<(std::ostream &out, const ServerInfo::Server &value) {
  return out << REDIS_SERVER_VERSION_LABEL ":" << value.redis_version_
             << REDIS_INFO_MARKER << REDIS_SERVER_GIT_SHA1_LABEL ":"
             << value.redis_git_sha1_ << REDIS_INFO_MARKER
             << REDIS_SERVER_GIT_DIRTY_LABEL ":" << value.redis_git_dirty_
             << REDIS_INFO_MARKER << REDIS_SERVER_MODE_LABEL ":"
             << value.redis_mode_ << REDIS_INFO_MARKER
             << REDIS_SERVER_OS_LABEL ":" << value.os_ << REDIS_INFO_MARKER
             << REDIS_SERVER_ARCH_BITS_LABEL ":" << value.arch_bits_
             << REDIS_INFO_MARKER << REDIS_SERVER_MULTIPLEXING_API_LABEL ":"
             << value.multiplexing_api_ << REDIS_INFO_MARKER
             << REDIS_SERVER_GCC_VERSION_LABEL ":" << value.gcc_version_
             << REDIS_INFO_MARKER << REDIS_SERVER_PROCESS_ID_LABEL ":"
             << value.process_id_ << REDIS_INFO_MARKER
             << REDIS_SERVER_RUN_ID_LABEL ":" << value.run_id_
             << REDIS_INFO_MARKER << REDIS_SERVER_TCP_PORT_LABEL ":"
             << value.tcp_port_ << REDIS_INFO_MARKER
             << REDIS_SERVER_UPTIME_IN_SECONDS_LABEL ":"
             << value.uptime_in_seconds_ << REDIS_INFO_MARKER
             << REDIS_SERVER_UPTIME_IN_DAYS_LABEL ":" << value.uptime_in_days_
             << REDIS_INFO_MARKER << REDIS_SERVER_HZ_LABEL ":" << value.hz_
             << REDIS_INFO_MARKER << REDIS_SERVER_LRU_CLOCK_LABEL ":"
             << value.lru_clock_ << REDIS_INFO_MARKER;
}

std::ostream &operator<<(std::ostream &out, const ServerInfo::Clients &value) {
  return out << REDIS_CLIENTS_CONNECTED_CLIENTS_LABEL ":"
             << value.connected_clients_ << REDIS_INFO_MARKER
             << REDIS_CLIENTS_CLIENT_LONGEST_OUTPUT_LIST_LABEL ":"
             << value.client_longest_output_list_ << REDIS_INFO_MARKER
             << REDIS_CLIENTS_CLIENT_BIGGEST_INPUT_BUF_LABEL ":"
             << value.client_biggest_input_buf_ << REDIS_INFO_MARKER
             << REDIS_CLIENTS_BLOCKED_CLIENTS_LABEL ":"
             << value.blocked_clients_ << REDIS_INFO_MARKER;
}

std::ostream &operator<<(std::ostream &out, const ServerInfo::Memory &value) {
  return out << REDIS_MEMORY_USED_MEMORY_LABEL ":" << value.used_memory_
             << REDIS_INFO_MARKER << REDIS_MEMORY_USED_MEMORY_HUMAN_LABEL ":"
             << value.used_memory_human_ << REDIS_INFO_MARKER
             << REDIS_MEMORY_USED_MEMORY_RSS_LABEL ":" << value.used_memory_rss_
             << REDIS_INFO_MARKER << REDIS_MEMORY_USED_MEMORY_PEAK_LABEL ":"
             << value.used_memory_peak_ << REDIS_INFO_MARKER
             << REDIS_MEMORY_USED_MEMORY_PEAK_HUMAN_LABEL ":"
             << value.used_memory_peak_human_ << REDIS_INFO_MARKER
             << REDIS_MEMORY_USED_MEMORY_LUA_LABEL ":" << value.used_memory_lua_
             << REDIS_INFO_MARKER
             << REDIS_MEMORY_MEM_FRAGMENTATION_RATIO_LABEL ":"
             << value.mem_fragmentation_ratio_ << REDIS_INFO_MARKER
             << REDIS_MEMORY_MEM_ALLOCATOR_LABEL ":" << value.mem_allocator_
             << REDIS_INFO_MARKER;
}

std::ostream &operator<<(std::ostream &out,
                         const ServerInfo::Persistence &value) {
  return out << REDIS_PERSISTENCE_LOADING_LABEL ":" << value.loading_
             << REDIS_INFO_MARKER
             << REDIS_PERSISTENCE_RDB_CHANGES_SINCE_LAST_SAVE_LABEL ":"
             << value.rdb_changes_since_last_save_ << REDIS_INFO_MARKER
             << REDIS_PERSISTENCE_RDB_DGSAVE_IN_PROGRESS_LABEL ":"
             << value.rdb_bgsave_in_progress_ << REDIS_INFO_MARKER
             << REDIS_PERSISTENCE_RDB_LAST_SAVE_TIME_LABEL ":"
             << value.rdb_last_save_time_ << REDIS_INFO_MARKER
             << REDIS_PERSISTENCE_RDB_LAST_DGSAVE_STATUS_LABEL ":"
             << value.rdb_last_bgsave_status_ << REDIS_INFO_MARKER
             << REDIS_PERSISTENCE_RDB_LAST_DGSAVE_TIME_SEC_LABEL ":"
             << value.rdb_last_bgsave_time_sec_ << REDIS_INFO_MARKER
             << REDIS_PERSISTENCE_RDB_CURRENT_DGSAVE_TIME_SEC_LABEL ":"
             << value.rdb_current_bgsave_time_sec_ << REDIS_INFO_MARKER
             << REDIS_PERSISTENCE_AOF_ENABLED_LABEL ":" << value.aof_enabled_
             << REDIS_INFO_MARKER
             << REDIS_PERSISTENCE_AOF_REWRITE_IN_PROGRESS_LABEL ":"
             << value.aof_rewrite_in_progress_ << REDIS_INFO_MARKER
             << REDIS_PERSISTENCE_AOF_REWRITE_SHEDULED_LABEL ":"
             << value.aof_rewrite_scheduled_ << REDIS_INFO_MARKER
             << REDIS_PERSISTENCE_AOF_LAST_REWRITE_TIME_SEC_LABEL ":"
             << value.aof_last_rewrite_time_sec_ << REDIS_INFO_MARKER
             << REDIS_PERSISTENCE_AOF_CURRENT_REWRITE_TIME_SEC_LABEL ":"
             << value.aof_current_rewrite_time_sec_ << REDIS_INFO_MARKER
             << REDIS_PERSISTENCE_AOF_LAST_DGREWRITE_STATUS_LABEL ":"
             << value.aof_last_bgrewrite_status_ << REDIS_INFO_MARKER
             << REDIS_PERSISTENCE_AOF_LAST_WRITE_STATUS_LABEL ":"
             << value.aof_last_write_status_ << REDIS_INFO_MARKER;
}

std::ostream &operator<<(std::ostream &out, const ServerInfo::Stats &value) {
  return out << REDIS_STATS_TOTAL_CONNECTIONS_RECEIVED_LABEL ":"
             << value.total_connections_received_ << REDIS_INFO_MARKER
             << REDIS_STATS_TOTAL_COMMANDS_PROCESSED_LABEL ":"
             << value.total_commands_processed_ << REDIS_INFO_MARKER
             << REDIS_STATS_INSTANTANEOUS_OPS_PER_SEC_LABEL ":"
             << value.instantaneous_ops_per_sec_ << REDIS_INFO_MARKER
             << REDIS_STATS_REJECTED_CONNECTIONS_LABEL ":"
             << value.rejected_connections_ << REDIS_INFO_MARKER
             << REDIS_STATS_SYNC_FULL_LABEL ":" << value.sync_full_
             << REDIS_INFO_MARKER << REDIS_STATS_SYNC_PARTIAL_OK_LABEL ":"
             << value.sync_partial_ok_ << REDIS_INFO_MARKER
             << REDIS_STATS_SYNC_PARTIAL_ERR_LABEL ":"
             << value.sync_partial_err_ << REDIS_INFO_MARKER
             << REDIS_STATS_EXPIRED_KEYS_LABEL ":" << value.expired_keys_
             << REDIS_INFO_MARKER << REDIS_STATS_EVICTED_KEYS_LABEL ":"
             << value.evicted_keys_ << REDIS_INFO_MARKER
             << REDIS_STATS_KEYSPACE_HITS_LABEL ":" << value.keyspace_hits_
             << REDIS_INFO_MARKER << REDIS_STATS_KEYSPACE_MISSES_LABEL ":"
             << value.keyspace_misses_ << REDIS_INFO_MARKER
             << REDIS_STATS_PUBSUB_CHANNELS_LABEL ":" << value.pubsub_channels_
             << REDIS_INFO_MARKER << REDIS_STATS_PUBSUB_PATTERNS_LABEL ":"
             << value.pubsub_patterns_ << REDIS_INFO_MARKER
             << REDIS_STATS_LATEST_FORK_USEC_LABEL ":"
             << value.latest_fork_usec_ << REDIS_INFO_MARKER;
}

std::ostream &operator<<(std::ostream &out,
                         const ServerInfo::Replication &value) {
  return out << REDIS_REPLICATION_ROLE_LABEL ":" << value.role_
             << REDIS_INFO_MARKER
             << REDIS_REPLICATION_CONNECTED_SLAVES_LABEL ":"
             << value.connected_slaves_ << REDIS_INFO_MARKER
             << REDIS_REPLICATION_MASTER_REPL_OFFSET_LABEL ":"
             << value.master_repl_offset_ << REDIS_INFO_MARKER
             << REDIS_REPLICATION_BACKLOG_ACTIVE_LABEL ":"
             << value.backlog_active_ << REDIS_INFO_MARKER
             << REDIS_REPLICATION_BACKLOG_SIZE_LABEL ":" << value.backlog_size_
             << REDIS_INFO_MARKER
             << REDIS_REPLICATION_BACKLOG_FIRST_BYTE_OFFSET_LABEL ":"
             << value.backlog_first_byte_offset_ << REDIS_INFO_MARKER
             << REDIS_REPLICATION_BACKLOG_HISTEN_LABEL ":"
             << value.backlog_histen_ << REDIS_INFO_MARKER;
}

std::ostream &operator<<(std::ostream &out, const ServerInfo::Cpu &value) {
  return out << REDIS_CPU_USED_CPU_SYS_LABEL ":" << value.used_cpu_sys_
             << REDIS_INFO_MARKER << REDIS_CPU_USED_CPU_USER_LABEL ":"
             << value.used_cpu_user_ << REDIS_INFO_MARKER
             << REDIS_CPU_USED_CPU_SYS_CHILDREN_LABEL ":"
             << value.used_cpu_sys_children_ << REDIS_INFO_MARKER
             << REDIS_CPU_USED_CPU_USER_CHILDREN_LABEL ":"
             << value.used_cpu_user_children_ << REDIS_INFO_MARKER;
}

std::string ServerInfo::ToString() const {
  std::stringstream str;
  str << REDIS_SERVER_LABEL REDIS_INFO_MARKER << server_
      << REDIS_CLIENTS_LABEL REDIS_INFO_MARKER << clients_
      << REDIS_MEMORY_LABEL REDIS_INFO_MARKER << memory_
      << REDIS_PERSISTENCE_LABEL REDIS_INFO_MARKER << persistence_
      << REDIS_STATS_LABEL REDIS_INFO_MARKER << stats_
      << REDIS_REPLICATION_LABEL REDIS_INFO_MARKER << replication_
      << REDIS_CPU_LABEL REDIS_INFO_MARKER << cpu_
...
ServerInfo *MakeRedisServerInfo(const std::string &content) {
  ...
  std::string word;
  size_t pos = 0;
  static const std::vector<core::info_field_t> fields =
      DBTraits<REDIS>::GetInfoFields();
