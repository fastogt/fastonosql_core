...
const std::vector<Field> g_pika_log_fields = {
    Field(PIKA_LOG_SIZE_LABEL, common::Value::TYPE_UINTEGER),
...
const std::vector<Field> g_pika_stats_fields = {
    Field(PIKA_STATS_TOTAL_CONNECTIONS_RECEIVED_LABEL,
          common::Value::TYPE_UINTEGER),
    Field(PIKA_STATS_INSTANTANEOUS_OPS_PER_SEC_LABEL,
          common::Value::TYPE_UINTEGER),
    Field(PIKA_STATS_TOTAL_COMMANDS_PROCESSED_LABEL,
          common::Value::TYPE_UINTEGER),
...
const std::vector<Field> g_redis_replication_fields = {
    ...
    Field(PIKA_REPLICATION_CONNECTED_SLAVES_LABEL,
          common::Value::TYPE_UINTEGER)};
...
template <> std::vector<info_field_t> DBTraits<PIKA>::GetInfoFields() {
...
ServerInfo::Server::Server::Server()
    : pika_version_(), pika_git_sha_(), pika_build_compile_date_(), os_(),
      arch_bits_(), process_id_(), tcp_port_(), thread_num_(),
      sync_thread_num_(), uptime_in_seconds_(), uptime_in_days_(),
      config_file_(), server_id_() {}

ServerInfo::Server::Server(const std::string &server_text) : Server() {
  size_t pos = 0;
  size_t start = 0;
  while ((pos = server_text.find(PIKA_INFO_MARKER, start)) !=
         std::string::npos) {
...
ServerInfo::Data::Data()
    : db_size_(), db_size_human_(), compression_(), used_memory_(),
      used_memory_human_(), db_memtable_usage_(), db_tablereader_usage_() {}

ServerInfo::Data::Data(const std::string &data_text) : Data() {
  size_t pos = 0;
  size_t start = 0;
  while ((pos = data_text.find((PIKA_INFO_MARKER), start)) !=
         std::string::npos) {
...
ServerInfo::Log::Log()
    : log_size_(), log_size_human_(), safety_purge_(), expire_logs_days_(),
      expire_logs_nums_(), binlog_offset_() {}

ServerInfo::Log::Log(const std::string &log_text) : Log() {
  size_t pos = 0;
  size_t start = 0;
  while ((pos = log_text.find((PIKA_INFO_MARKER), start)) !=
         std::string::npos) {
...
ServerInfo::Clients::Clients(const std::string &client_text) : Clients() {
  size_t pos = 0;
  size_t start = 0;
  while ((pos = client_text.find((PIKA_INFO_MARKER), start)) !=
         std::string::npos) {
...
ServerInfo::Hub::Hub(const std::string &hub_text) : Hub() { UNUSED(hub_text); }
...
ServerInfo::Stats::Stats()
    : total_connections_received_(), instantaneous_ops_per_sec_(),
      total_commands_processed_(), is_bgsaving_(), is_slots_reloading_(),
      is_slots_cleanuping_(), is_scaning_keyspace_(), is_compact_(),
      compact_cron_(), compact_interval_() {}

ServerInfo::Stats::Stats(const std::string &stats_text) : Stats() {
  size_t pos = 0;
  size_t start = 0;
  while ((pos = stats_text.find((PIKA_INFO_MARKER), start)) !=
         std::string::npos) {
...
ServerInfo::Cpu::Cpu(const std::string &cpu_text) : Cpu() {
  size_t pos = 0;
  size_t start = 0;
  while ((pos = cpu_text.find((PIKA_INFO_MARKER), start)) !=
         std::string::npos) {
...
ServerInfo::Replication::Replication(const std::string &replication_text)
  ...
  while ((pos = replication_text.find((PIKA_INFO_MARKER), start)) !=
         std::string::npos) {
...
ServerInfo::DoubleMaster::DoubleMaster(const std::string &dm_text)
    : DoubleMaster() {
  UNUSED(dm_text);
}

common::Value *
ServerInfo::DoubleMaster::GetValueByIndex(unsigned char index) const {
  UNUSED(index);
  return nullptr;
}
...
ServerInfo::ServerInfo(const Server &serv, const Data &data, const Log &log,
                       const Clients &clients, struct Hub &hub,
                       const Stats &stats, const Cpu &cpu,
                       const Replication &repl, const KeySpace &key_space,
                       const DoubleMaster &double_master)
    : IServerInfo(PIKA), server_(serv), data_(data), log_(log),
      clients_(clients), hub_(hub), stats_(stats), cpu_(cpu),
      replication_(repl), key_space_(key_space), double_master_(double_master) {
}
...
std::ostream &operator<<(std::ostream &out, const ServerInfo::Server &value) {
  return out << PIKA_SERVER_VERSION_LABEL ":" << value.pika_version_
             << PIKA_INFO_MARKER << PIKA_SERVER_GIT_SHA_LABEL ":"
             << value.pika_git_sha_ << PIKA_INFO_MARKER
             << PIKA_SERVER_BUILD_COMPILE_DATE_LABEL ":"
             << value.pika_build_compile_date_ << PIKA_INFO_MARKER
             << PIKA_SERVER_OS_LABEL ":" << value.os_ << PIKA_INFO_MARKER
             << PIKA_SERVER_ARCH_BITS_LABEL ":" << value.arch_bits_
             << PIKA_INFO_MARKER << PIKA_SERVER_PROCESS_ID_LABEL ":"
             << value.process_id_ << PIKA_INFO_MARKER
             << PIKA_SERVER_TCP_PORT_LABEL ":" << value.tcp_port_
             << PIKA_INFO_MARKER << PIKA_SERVER_THREAD_NUM_LABEL ":"
             << value.thread_num_ << PIKA_INFO_MARKER
             << PIKA_SERVER_SYNC_THREAD_NUM_LABEL ":" << value.sync_thread_num_
             << PIKA_INFO_MARKER << PIKA_SERVER_UPTIME_IN_SECONDS_LABEL ":"
             << value.uptime_in_seconds_ << PIKA_INFO_MARKER
             << PIKA_SERVER_UPTIME_IN_DAYS_LABEL ":" << value.uptime_in_days_
             << PIKA_INFO_MARKER << PIKA_SERVER_CONFIG_FILE_LABEL ":"
             << value.config_file_ << PIKA_INFO_MARKER
             << PIKA_SERVER_SERVER_ID_LABEL ":" << value.server_id_
             << PIKA_INFO_MARKER;
}

std::ostream &operator<<(std::ostream &out, const ServerInfo::Data &value) {
  return out << PIKA_DATA_DB_SIZE_LABEL ":" << value.db_size_
             << PIKA_INFO_MARKER << PIKA_DATA_DB_SIZE_HUMAN_LABEL ":"
             << value.db_size_human_ << PIKA_INFO_MARKER
             << PIKA_DATA_COMPRESSION_LABEL ":" << value.compression_
             << PIKA_INFO_MARKER << PIKA_DATA_USED_MEMORY_LABEL ":"
             << value.used_memory_ << PIKA_INFO_MARKER
             << PIKA_DATA_USED_MEMORY_HUMAN_LABEL ":"
             << value.used_memory_human_ << PIKA_INFO_MARKER
             << PIKA_DATA_DB_MEMTABLE_USAGE_LABEL ":"
             << value.db_memtable_usage_ << PIKA_INFO_MARKER
             << PIKA_DATA_DB_TABLEREADER_USAGE_LABEL ":"
             << value.db_tablereader_usage_ << PIKA_INFO_MARKER;
}

std::ostream &operator<<(std::ostream &out, const ServerInfo::Log &value) {
  return out << PIKA_LOG_SIZE_LABEL ":" << value.log_size_ << PIKA_INFO_MARKER
             << PIKA_LOG_SIZE_HUMAN_LABEL ":" << value.log_size_human_
             << PIKA_INFO_MARKER << PIKA_LOG_SAFETY_PURGE_LABEL ":"
             << value.safety_purge_ << PIKA_INFO_MARKER
             << PIKA_LOG_EXPIRE_LOGS_DAYS_LABEL ":" << value.expire_logs_days_
             << PIKA_INFO_MARKER << PIKA_LOG_EXPIRE_LOGS_NUMS_LABEL ":"
             << value.expire_logs_nums_ << PIKA_INFO_MARKER
             << PIKA_LOG_BINLOG_OFFSET_LABEL ":" << value.binlog_offset_
             << PIKA_INFO_MARKER;
}

std::ostream &operator<<(std::ostream &out, const ServerInfo::Clients &value) {
  return out << PIKA_CLIENTS_CONNECTED_CLIENTS_LABEL ":"
             << value.connected_clients_ << PIKA_INFO_MARKER;
}
...
std::ostream &operator<<(std::ostream &out, const ServerInfo::Stats &value) {
  return out << PIKA_STATS_TOTAL_CONNECTIONS_RECEIVED_LABEL ":"
             << value.total_connections_received_ << PIKA_INFO_MARKER
             << PIKA_STATS_INSTANTANEOUS_OPS_PER_SEC_LABEL ":"
             << value.instantaneous_ops_per_sec_ << PIKA_INFO_MARKER
             << PIKA_STATS_TOTAL_COMMANDS_PROCESSED_LABEL ":"
             << value.total_commands_processed_ << PIKA_INFO_MARKER
             << PIKA_STATS_IS_BGSAVING_LABEL ":" << value.is_bgsaving_
             << PIKA_INFO_MARKER << PIKA_STATS_IS_SLOTS_RELOADING_LABEL ":"
             << value.is_slots_reloading_ << PIKA_INFO_MARKER
             << PIKA_STATS_IS_SLOTS_CLEANUPING_LABEL ":"
             << value.is_slots_cleanuping_ << PIKA_INFO_MARKER
             << PIKA_STATS_IS_SCANING_KEYSPACE_LABEL ":"
             << value.is_scaning_keyspace_ << PIKA_INFO_MARKER
             << PIKA_STATS_IS_COMPACT_LABEL ":" << value.is_compact_
             << PIKA_INFO_MARKER << PIKA_STATS_COMPACT_CRON_LABEL ":"
             << value.compact_cron_ << PIKA_INFO_MARKER
             << PIKA_STATS_COMPACT_INTERVAL_LABEL ":" << value.compact_interval_
             << PIKA_INFO_MARKER;
}

std::ostream &operator<<(std::ostream &out, const ServerInfo::Cpu &value) {
  return out << PIKA_CPU_USED_CPU_SYS_LABEL ":" << value.used_cpu_sys_
             << PIKA_INFO_MARKER << PIKA_CPU_USED_CPU_USER_LABEL ":"
             << value.used_cpu_user_ << PIKA_INFO_MARKER
             << PIKA_CPU_USED_CPU_SYS_CHILDREN_LABEL ":"
             << value.used_cpu_sys_children_ << PIKA_INFO_MARKER
             << PIKA_CPU_USED_CPU_USER_CHILDREN_LABEL ":"
             << value.used_cpu_user_children_ << PIKA_INFO_MARKER;
}

std::ostream &operator<<(std::ostream &out,
                         const ServerInfo::Replication &value) {
  return out << PIKA_REPLICATION_ROLE_LABEL ":" << value.role_
             << PIKA_INFO_MARKER << PIKA_REPLICATION_CONNECTED_SLAVES_LABEL ":"
             << value.connected_slaves_ << PIKA_INFO_MARKER;
}

std::ostream &operator<<(std::ostream &out, const ServerInfo::KeySpace &value) {
  return out << PIKA_KEYSPACE_KV_KEYS_LABEL ":" << value.kv_ << PIKA_INFO_MARKER
             << PIKA_KEYSPACE_HASH_KEYS_LABEL ":" << value.hash_
             << PIKA_INFO_MARKER << PIKA_KEYSPACE_LIST_KEYS_LABEL ":"
             << value.list_ << PIKA_INFO_MARKER
             << PIKA_KEYSPACE_ZSET_KEYS_LABEL ":" << value.zset_
             << PIKA_INFO_MARKER << PIKA_KEYSPACE_SET_KEYS_LABEL ":"
             << value.set_ << PIKA_INFO_MARKER;
}

std::ostream &operator<<(std::ostream &out,
                         const ServerInfo::DoubleMaster &value) {
  UNUSED(value);
  return out << PIKA_INFO_MARKER;
}
...
std::string ServerInfo::ToString() const {
  ...
  str << PIKA_SERVER_LABEL PIKA_INFO_MARKER << server_
      << PIKA_DATA_LABEL PIKA_INFO_MARKER << data_
      << PIKA_LOG_LABEL PIKA_INFO_MARKER << log_
      << PIKA_CLIENTS_LABEL PIKA_INFO_MARKER << clients_
      << PIKA_HUB_LABEL PIKA_INFO_MARKER << hub_
      << PIKA_STATS_LABEL PIKA_INFO_MARKER << stats_
      << PIKA_CPU_LABEL PIKA_INFO_MARKER << cpu_
      << PIKA_REPLICATION_LABEL PIKA_INFO_MARKER << replication_
      << PIKA_KEYSPACE_LABEL PIKA_INFO_MARKER << key_space_
      << PIKA_DOUBLE_MASTER_LABE PIKA_INFO_MARKER << double_master_
      << PIKA_INFO_MARKER;
  return str.str();
}
