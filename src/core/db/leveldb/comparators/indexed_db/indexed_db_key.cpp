...
IndexedDBKey::IndexedDBKey(const KeyArray &array)
    : type_(kWebIDBKeyTypeArray), array_(CopyKeyArray(array)),
      size_estimate_(kOverheadSize + CalculateArraySize(array)) {}
IndexedDBKey::IndexedDBKey(const std::string &binary)
    : type_(kWebIDBKeyTypeBinary), binary_(binary),
      size_estimate_(kOverheadSize +
                     (binary.length() * sizeof(std::string::value_type))) {}
IndexedDBKey::IndexedDBKey(const common::string16 &string)
    : type_(kWebIDBKeyTypeString), string_(string),
      size_estimate_(kOverheadSize +
                     (string.length() * sizeof(common::string16::value_type))) {
}
