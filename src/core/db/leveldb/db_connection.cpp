...
template <> const char *ConnectionTraits<LEVELDB>::GetBasedOn() {
  return "leveldb";
}

template <> const char *ConnectionTraits<LEVELDB>::GetVersionApi() {
  static std::string leveldb_version = common::MemSPrintf(
      "%d.%d", leveldb_major_version(), leveldb_minor_version());
...
template <>
common::Error
ConnectionAllocatorTraits<leveldb::NativeConnection, leveldb::Config>::Connect(
    const leveldb::Config &config, leveldb::NativeConnection **hout) {
...
template <>
common::Error
ConnectionAllocatorTraits<leveldb::NativeConnection, leveldb::Config>::
    Disconnect(leveldb::NativeConnection **handle) {
  destroy(handle);
  return common::Error();
}

template <>
bool ConnectionAllocatorTraits<leveldb::NativeConnection, leveldb::Config>::
    IsConnected(leveldb::NativeConnection *handle) {
  if (!handle) {
    return false;
  }
...
common::Error CreateConnection(const Config &config,
  ...
  if (!st.ok()) {
    std::string buff =
        common::MemSPrintf("Fail connect to server: %s!", st.ToString());
...
common::Error DBConnection::KeysImpl(const std::string &key_start,
  ...
  ::leveldb::ReadOptions ro;
  ::leveldb::Iterator *it = connection_.handle_->NewIterator(
