...
#define UNQLITE_SET_KEY_COMMAND DB_SET_KEY_COMMAND
#define UNQLITE_DELETE_KEY_COMMAND DB_DELETE_KEY_COMMAND
#define UNQLITE_RENAME_KEY_COMMAND DB_RENAME_KEY_COMMAND
...
common::Error CommandTranslator::CreateKeyCommandImpl(const NDbKValue& key, command_buffer_t* cmdstring) const {
  ...
  wr << UNQLITE_SET_KEY_COMMAND SPACE_STR << key_str.GetForCommandLine() << SPACE_STR << value.GetForCommandLine();
  *cmdstring = wr.str();
  return common::Error();
}
...
bool CommandTranslator::IsLoadKeyCommandImpl(const CommandInfo& cmd) const {
  return cmd.IsEqualName(GEN_CMD_STRING(UNQLITE_GET_KEY_COMMAND));
}
