...
template <> std::vector<info_field_t> DBTraits<UNQLITE>::GetInfoFields() {
...
std::ostream &operator<<(std::ostream &out, const ServerInfo::Stats &value) {
  return out << UNQLITE_STATS_DB_FILE_PATH_LABEL ":" << value.db_path << MARKER
             << UNQLITE_STATS_DB_FILE_SIZE_LABEL ":" << value.db_size << MARKER;
}
...
ServerInfo *MakeUnqliteServerInfo(const std::string &content) {
  ...
  static const std::vector<info_field_t> fields =
      DBTraits<UNQLITE>::GetInfoFields();
  std::string word;
